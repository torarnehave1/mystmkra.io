  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        !metadata.result.shared_folder_id
      ) {
        // Share the folder if it is not shared yet
        const shareResponse = await dbx.sharingShareFolder({
          path: folderPath,
        });
      ...
      if (
        ...
        error.error &&
        error.error.error_summary &&
        error.error.error_summary.includes("email_unverified")
      ) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const sharedFolder = sharedFoldersList.result.entries.find(
        (folder) => folder.path_lower === folderPath.toLowerCase()
      );
      ...
      if (error.status === 409) {
        if (error.error.error_summary.includes("not_found")) {
          ...
          error.error.error_summary.includes("invalid_shared_folder_id")
        ) {
          errorMessage =
            "The folder is not shared and cannot be accessed as a shared folder.";
        }
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (getResponse.data.matches && getResponse.data.matches.length > 0) {
        ...
        const field = getResponse.data.matches[0].properties[0].fields.find(
          (f) => f.name === "tag"
        );
...
router.get("/list-image-files", ensureValidToken, async (req, res) => {
  ...
  try {
    ...
    const fileEntriesWithLinks = await Promise.all(
      entries.map(async (entry) => {
        if (entry[".tag"] === "file") {
          try {
            ...
            console.error(
              "Error getting temporary link for file:",
              entry.name,
              error
            );
            return null;
    ...
    try {
      ...
      if (process.env.NODE_ENV === "production") {
        if (imageUrlFromMarkdown.includes("localhost")) {
          imageUrlFromMarkdown = imageUrlFromMarkdown.replace(
            "http://localhost:3001",
            baseUrl
          );
        }
      ...
      // Preprocess custom blocks before markdown rendering
      function preprocessCustomBlocks(markdown) {
        // [SECTION]
        markdown = markdown.replace(/\[SECTION\s*\|([^\]]+)\]([\s\S]*?)\[END SECTION\]/g, (match, style, content) => {
          const css = style
            .split(';')
            .map((s) => s.trim())
            .filter(Boolean)
        ...
        // [QUOTE]
        markdown = markdown.replace(/\[QUOTE\s*\|([^\]]+)\]([\s\S]*?)\[END QUOTE\]/g, (match, style, content) => {
          const processedContent = marked.parse(content.trim());
        ...
        // [WNOTE]
        markdown = markdown.replace(/\[WNOTE\s*\|([^\]]+)\]([\s\S]*?)\[END WNOTE\]/g, (match, style, content) => {
          const processedContent = marked.parse(content.trim());
        ...
        // [FANCY]
        markdown = markdown.replace(/\[FANCY\s*\|([^\]]+)\]([\s\S]*?)\[END FANCY\]/g, (match, style, content) => {
          const css = style
            .split(';')
            .map((s) => s.trim())
            .filter(Boolean)
        ...
        // [YOUTUBE]
        markdown = markdown.replace(/!\[YOUTUBE src=(.+?)\](.+?)\[END YOUTUBE\]/g, (match, src, title) => {
...
router.get(
  "/blogoriginal/:userFolder/:filename",
  ensureValidToken,
  async (req, res) => {
    const userFolder = req.params.userFolder;
    let filename = req.params.filename;
...
router.get("/md/topdf/:filename", ensureValidToken, async (req, res) => {
  ...
  try {
    ...
    const pythonProcess = spawn("python", [
      join(__dirname, "..", "modules", "micro", "htmltopdf.py"),
      outputPdfPath,
    ]);
    ...
    pythonProcess.on("close", async (code) => {
      ...
      if (code === 0) {
        ...
        const pdfDropboxPath = `/Slowyou.net/pdf/${filename.replace(
          ".md",
          ""
        )}.pdf`;
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (
        !shareFolderResponse.result ||
        !shareFolderResponse.result.shared_folder_id
      ) {
        return res.status(500).json({
