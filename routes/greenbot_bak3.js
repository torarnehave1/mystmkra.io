...
import { handleCreateProcessManual } from '../services/greenbot/createProcessManual.js';
...
import { presentStep, presentCategorySelection, handleNextReviewStep } from '../services/processService.js'; // Import from processService
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  */
  ...
  if (data.startsWith('start_process_')) {
    const processId = extractProcessIdFromCallbackData(data);
    console.log(`[DEBUG STA01] start_process callback triggered for process ${processId} by user ${chatId}`);
    const userState = await UserState.findOne({ userId: chatId });
    const process = await Process.findById(processId);
    const currentStep = process.steps[userState.currentStepIndex];
    handleNextReviewStep(bot, chatId, processId, currentStep, userState);
  }
