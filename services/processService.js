...
const presentReviewStep = async (bot, chatId, processId, currentStep, userState) => {
  ...
  // Conditionally add the "Previous Step" button
  const inlineKeyboard = [];
  if (stepIndex > 0) {
    inlineKeyboard.push([{ text: 'Previous Step', callback_data: `previous_review_step_${processId}_${stepIndex}` }]);
  }
  inlineKeyboard.push([{ text: 'Next Step', callback_data: `next_review_step_${processId}_${stepIndex}` }]);
